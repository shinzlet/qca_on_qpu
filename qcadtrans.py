...
class ParserNode:
    ...
    _rgx_bv  = re.compile('(\d+)')                                # bus data value (just an integer)
        ...
        for line in fp:
            if self._rgx_match(self._rgx_in, line, 1):
                ...
                if m is not None and m.lastindex==2:
                    ...
                    m = self._rgx_bv.match(line)
                    if m is not None and m.lastindex == 1:
                        # The tag will always be "BUS_DATA" and this is the
                        # only field stored within.
                        self.d["value"] = m.group(1)
...
class QCACircuit(Graph):
    ...
    def add_cell(self, x, y, scale=True, cf='normal', pol=0, rot=False, name=None):
        ...
        inputs:
            ...
            rot     : cell is rotated
            name    : cell's name, if it is an input or output
        '''
        ...
        self._add_cell(x, y, cf, pol, rot, dots, name)
    ...
    def _add_from_node(self, node):
        ...
        # get the cell name, if one exists (and the cell is an i/o)
        name = None
        if cf == _CFs['input'] or cf == _CFs['output']:
            for child in node.children:
                if child.d.get('psz'):
                    name = child.d.get('psz')
    ...
    def _add_cell(self, x, y, cf, pol, rot, dots, name):
        '''handler for adding a new cell to the circuit'''

        n = len(self)
        self.add_node(n, weight=0, x=x, y=y, cf=cf, pol=pol, rot=rot, dots=dots, name=name)
...
if __name__ == '__main__':
    ...
    for i in range(len(circuit.nodes)):
        ...
        # within each node.
        print(f'x: {node["x"]}\ty: {node["y"]}\tpol: {node["pol"]}\trotated: {node["rot"]}\tcf: {list(_CFs.keys())[node["cf"]]}\tname: {node["name"]}')
